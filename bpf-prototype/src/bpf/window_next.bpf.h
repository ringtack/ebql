...
// TODO: maybe change buffer size to a diff variable, and in time windows, make
// it a const volatile to update if too much data; I think we can run-time
// replace a map ^ But this requires a map-of-maps, which incurs significant
// runtime overhead w/ bounds checking... perhaps better idea is to perform PGO
// in query optimization (likely that this is necessary anyways for opt
// purposes), then adjust window size w/ a new program then??
...
// Appends an element to the window. If window flushing is needed, returns the
...
static u32 __always_inline window_add(window_t *w, simple_1_t q) {
  ...
  if (w->w_size < WINDOW_SIZE) {
    ...
    if (w->next_idx == STEP) {
      ...
      if (w->w_tail + STEP <= WINDOW_SIZE) {
        // We use bpf_probe_read_kernel, because __builtin_memcpy works only
        // when the amount to copy < 512B; otherwise, it exceeds the BPF stack
        // size limit.
        bpf_probe_read_kernel(w->expired, STEP * sizeof(simple_1_t),
                              &w->win[w->w_tail]);
      } else {
        ...
        bpf_probe_read_kernel(w->expired, n * sizeof(simple_1_t),
                              &w->win[w->w_tail]);
        // Copy over [0, STEP - n) aka STEP - n elements
        bpf_probe_read_kernel(&w->expired[n], (STEP - n) * sizeof(simple_1_t),
                              w->win);
      }
      // Advance tail
      ...
      if (w->w_head + STEP <= WINDOW_SIZE) {
        bpf_probe_read_kernel(&w->win[w->w_head], STEP * sizeof(simple_1_t),
                              w->next);
      } else {
        u32 n = WINDOW_SIZE - w->w_head;
        // Copy over first n elements to [head, WINDOW_SIZE)
        bpf_probe_read_kernel(&w->win[w->w_head], n * sizeof(simple_1_t),
                              w->next);
        // Copy over remaining elements to [0, STEP - n)
        bpf_probe_read_kernel(w->win, (STEP - n) * sizeof(simple_1_t),
                              &w->next[n]);
      }
      // Advance head
      ...
      // decreases; items stay in the window until they're pushed out by the
      // next step, but the total amount doesn't change
      expired = STEP;
    }
  }
  // {{ else }}
  ...
  if (t_since_oldest < INTERVAL) {
    ...
    // Otherwise, first check if more than (INTERVAL+STEP) time has elapsed
    // since the oldest element in the window; if so, flush all elements and
    ...
    if (t_since_oldest > (INTERVAL + STEP)) {
      ...
      if (INTERVAL == STEP) {
        ...
        for (u32 i = 0; i < WINDOW_SIZE; i++) {
          if (w->win[new_tail].time >= new_oldest_t)
            break;
      ...
      expired =
          (new_tail != w->w_tail)
              ...
              // If new_tail == old_tail, window is full, so copy over
              // everything
              : WINDOW_SIZE;
      // Check if we can copy over in one memcpy; can't do == here, since if
      // new_tail != 0, expired can be WINDOW_SIZE and so an overflow could
      // occur
      if (w->w_tail < new_tail) {
        bpf_probe_read_kernel(w->expired, expired * sizeof(simple_1_t),
                              &w->win[w->w_tail]);
      } else {
        u32 n = WINDOW_SIZE - w->w_tail;
        ...
        bpf_probe_read_kernel(&w->expired[n],
                              (expired - n) * sizeof(simple_1_t), w->win);
      }
      // Advance tail
      w->w_tail = new_tail;
      ...
      // Check if we can copy over in one memcpy; here next_idx == # of new
      // elements in next window
      if (w->w_head + w->next_idx <= WINDOW_SIZE) {
        bpf_probe_read_kernel(&w->win[w->w_head],
                              w->next_idx * sizeof(simple_1_t), w->next);
      } else {
        u32 n = WINDOW_SIZE - w->w_head;
        bpf_probe_read_kernel(&w->win[w->w_head], n * sizeof(simple_1_t),
                              w->next);
        // Appease the verifier, since it can't verify from L213 that next_idx >
        // n ...
        if (w->next_idx < n) {
      ...
      // Here, window size might change: window lost expired items, but gained
      // next_idx items
      w->w_size = w->w_size + w->next_idx - expired;
      // Reset next buffer back to beginning
      w->next_idx = 0;
    } else {
      // Check only necessary if we didn't step; otherwise next_idx goes back to
      // 0
