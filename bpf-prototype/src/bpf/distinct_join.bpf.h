...
// Theoretically, max number of joins is MIN(DISTINCT_MAX_ENTRIES_1,
// DISTINCT_MAX_ENTRIES_2), since each element could match with each other;
// however, this is very unlikely for even the distinct max entries to be hit,
// or that every element joins with every other.
...
make_distinct_join_result_simple_1_simple_2(simple_1_t *l, simple_2_t *r,
...
// Joins a simple_1_t element to simple_2_t, storing the result in *res if
// found.
static __always_inline void
distinct_join_elt_simple_2(simple_1_t *e, simple_1_simple_2_t *res) {
  // Check if e's join key exists in simple_2's distinct table
  simple_2_t *r =
      (simple_2_t *)bpf_map_lookup_elem(&distinct_simple_2, &e->pid);
...
// Joins a simple_2_t element to simple_1_t, storing the result in *res if
// found.
static __always_inline void
distinct_join_elt_simple_1(simple_2_t *e, simple_1_simple_2_t *res) {
  // Check if e's join key exists in simple_2's distinct table
  simple_1_t *l =
      (simple_1_t *)bpf_map_lookup_elem(&distinct_simple_1, &e->pid);
  // If it does exist, make result
  if (l) {
...
// Context and callback for computing distinct joins between simple_1 and
// simple_2.
typedef struct {
...
static s64 __distinct_join_simple_1_simple_2_callback(
    struct bpf_map *map, u64 *pfn, simple_1_t *l,
    distinct_join_simple_1_simple_2_ctx_t *ctx) {
  // Check if l's join key appears in simple_2's distinct synopsis
  simple_2_t *r =
      (simple_2_t *)bpf_map_lookup_elem(&distinct_simple_2, &l->pid);
...
/**
 * Joins the two tables, outputting results into the passed in buffer, returning
 * the number of elements joined on success, and a negative error code on
 * failure.
 */
static __always_inline s32
distinct_join_simple_1_simple_2(simple_1_simple_2_t *buf, u32 buf_sz) {
  distinct_join_simple_1_simple_2_ctx_t ctx = {
      .buf = buf,
      .buf_sz = buf_sz,
      .count = 0,
  ...
  bpf_for_each_map_elem(&distinct_simple_1,
                        __distinct_join_simple_1_simple_2_callback, &ctx, 0);
}

// Callback for distinct join count computation
static s64 __distinct_join_simple_1_simple_2_count_callback(struct bpf_map *map,
                                                            u64 *pfn,
                                                            simple_1_t *l,
                                                            u32 *count) {
...
static __always_inline u32 distinct_join_simple_1_simple_2_count() {
  u32 count = 0;
  bpf_for_each_map_elem(&distinct_simple_1,
                        __distinct_join_simple_1_simple_2_count_callback,
                        &count, 0);
  return count;
}