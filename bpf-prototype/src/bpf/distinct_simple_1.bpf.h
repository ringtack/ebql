...
static __always_inline s64 __tumble_distinct_simple_1_clear_callback(struct bpf_map *map, u64 *key,
                                                                     simple_1_t *q, void *unused) {
  *q = (simple_1_t){0};
  return 0;
}
static __always_inline s64 __tumble_distinct_simple_1_copy_callback(struct bpf_map *map, u64 *key,
                                                                    simple_1_t *q, void *unused) {
...
static __always_inline void tumble_distinct_simple_1() {
  ...
  bpf_for_each_map_elem(&distinct_simple_1, __tumble_distinct_simple_1_clear_callback, NULL, 0);
  // Then, copy elements over from avg_next to avg
  bpf_for_each_map_elem(&distinct_next_simple_1, __tumble_distinct_simple_1_copy_callback, NULL, 0);
}
